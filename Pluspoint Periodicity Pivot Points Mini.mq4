...
#property description "Pivot Point Indicator"
#property strict
...
int start() {
   ...
   //ObjectCreate("test",OBJ_LABEL,1,0,0); 
  ...
  // str+=  "Company: " + AccountCompany() + "\n";
   ...
   //drawLabel("Label_Open", OBJPROP_CORNER, 25, "0.0014", 10, "Tahoma", Aqua);
    ...
    if(Period() == PERIOD_M1){
      ...
      //ObjectCreate("arrorz", OBJ_ARROW, 0, TimeCurrent(), Low[2] - 0,0020);
      ...
      //ObjectSet("test", OBJPROP_YDISTANCE, 10);
      //ObjectSet("test", OBJPROP_YDISTANCE, 5);
      //ObjectSetText("test","Hello World",20,"Arial",Yellow);
      
       double s3a = pivot - support3;
       double s2a = pivot - support2;
       double s1a = pivot - support1;
       
       double ssum = s3a + s2a + s1a;
       
       double s3p = (s3a/ssum) * 100;
       double s2p = (s2a/ssum) * 100;
       double s1p = (s1a/ssum) * 100;
       
      /************************************/
      double r3a = resistance1 - pivot;
      double r2a = resistance2 - pivot;
      double r1a = resistance3 - pivot;
      
      double rsum = r3a + r2a + r1a;
      
      double r3p = (r3a/rsum) * 100;
      double r2p = (r2a/rsum) * 100;
      double r1p = (r1a/rsum) * 100;
           
      drawLowerChart("S3",-s3p,Support_Colour);
      drawLowerChart("S2",-s2p,Support_Colour);
      drawLowerChart("S1",-s1p,Support_Colour);
      
      drawLowerChart("R3",r3p,Resistance_Colour);
      drawLowerChart("R2",r2p,Resistance_Colour);
      drawLowerChart("R1",r1p,Resistance_Colour);
