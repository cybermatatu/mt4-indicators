...
#define OrderStr "BuySellIndicators EA"

extern string	SmaF 				= "---------------- SMA Filter";
extern int       SmaPeriod =     100;
extern string	RVI 				= "---------------- RVI";
...
string TradeLog="BuySellIndicators EA";
...
string Indicator_Setting = "---------- Indicator Setting";
...
for (int shift=1; shift<=limit; shift++) {
   ...
   MACD1 = iCustom(Symbol(),0,"MACD",FastEMA,SlowEMA,SignalSMA,0,shift);
   MACD2 = iCustom(Symbol(),0,"MACD",FastEMA,SlowEMA,SignalSMA,1,shift);
   RVI1 = iCustom(Symbol(),0,"RVI",ExtRVIPeriod,0,shift);
   RVI2 = iCustom(Symbol(),0,"RVI",ExtRVIPeriod,1,shift);

	if ((MACD1<0 && MACD1>MACD2) || (MACD1>0 && MACD1>MACD2) && RVI1>RVI2 && Open[shift]>Sma)
      {	
...
if ((SellValue>0) ||
   (RealStopLoss_Enabled && (OrderOpenPrice() - Bid)/Point >= RealStopLoss) || (RealTakeProfit_Enabled && (Ask - OrderOpenPrice())/Point >= RealTakeProfit)){
	CloseBuy=1;
}


if ((BuyValue>0) ||
   (RealStopLoss_Enabled && (Ask - OrderOpenPrice())/Point >= RealStopLoss) ||(RealTakeProfit_Enabled && (OrderOpenPrice() - Bid)/Point >= RealTakeProfit)){
	CloseSell=1;
}
...
void subPrintDetails()
{
   ...
   string NL         = "\n";
	string sDirection = "";
   sComment = "BuySellIndicators EA" + NL;
